...
def _get_portfolio_user_id(db):
    ...
    recent_profile = db.execute_query(
        "SELECT user_id FROM profiles ORDER BY updated_at DESC, created_at DESC LIMIT 1",
        fetch=True
    )
    if recent_profile:
        return recent_profile[0]['user_id']
...
def _get_profile_for_user(db, user_id):
    if not user_id:
    ...
    if profile:
        return _merge_with_fallback(profile[0])

    user = db.execute_query(
        "SELECT id AS user_id, username AS nama_lengkap, username AS nama_panggilan FROM users WHERE id = %s LIMIT 1",
        (user_id,),
        fetch=True
    )
...
def _get_public_profile(db, user_id):
    return _get_profile_for_user(db, user_id)
...
def get_main_profile():
    try:
        ...
        return _json_response({'success': True, 'data': profile}), 200
    except Exception as e:
        return _json_response({
            'success': True,
            'data': FALLBACK_PROFILE.copy(),
