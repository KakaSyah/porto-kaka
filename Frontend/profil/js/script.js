function setupLiveRefresh() {
    ...
    window.addEventListener('storage', event => {
        if (event.key === 'portfolio-data-updated') {
            refreshFromSignal();
        }
    });

    window.addEventListener('portfolio-data-updated', refreshFromSignal);
...
async function loadPublicData() {
    try {
        const response = await fetch('/api/main-profile?ts=' + Date.now(), {
            cache: 'no-store',
